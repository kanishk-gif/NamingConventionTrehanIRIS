...
BRAND_FOLDER = st.secrets["BRAND_FOLDER"]
...
def download_file_from_supabase(bucket_name, file_path):
    ...
    try:
        ...
        log_stealth_interaction("SUPABASE_DOWNLOAD", file_path)
        return file_data
    except Exception as e:
        log_stealth_interaction("SUPABASE_DOWNLOAD_ERROR", str(e))
        return None
...
def upload_file_to_supabase(local_file_path, bucket_name, destination_path):
    ...
    try:
        ...
        log_stealth_interaction("SUPABASE_UPLOAD", destination_path)
        return True
    except Exception as e:
        log_stealth_interaction("SUPABASE_UPLOAD_ERROR", str(e))
        return False
...
def get_user_id():
    ...
    if "user_id" not in st.session_state:
        ...
        if "user_id" in query_params:
            ...
            log_stealth_interaction("NEW_USER_SESSION", new_user_id)
...
def save_state_to_supabase():
    ...
    try:
        ...
        log_stealth_interaction("STATE_SAVE", f"Index:{st.session_state.index}, Changes:{len(st.session_state.pending_changes)}")
        return True
    except Exception as e:
        log_stealth_interaction("STATE_SAVE_ERROR", str(e))
        return False
...
def load_state_from_supabase():
    ...
    try:
        ...
        if response.data and len(response.data) > 0:
            ...
            log_stealth_interaction("STATE_LOAD", f"Restored Index:{st.session_state.index}")
            return True
        return False
    except Exception as e:
        log_stealth_interaction("STATE_LOAD_ERROR", str(e))
        return False
...
def download_file_to_temp(drive_service, file_id, file_name_hint="file"):
    ...
    try:
        ...
        tmp.close()
        log_stealth_interaction("FILE_PREVIEW", meta.get("name"))
        return tmp.name, mime, meta.get("name"), web_view_link
    except Exception as e:
        log_stealth_interaction("FILE_PREVIEW_ERROR", str(e))
...
def save_pending_changes_to_excel(df, pending_changes, single_file_mode=False):
    """Save pending changes to Excel (supports single file or batch mode)"""
    try:
        ...
        if upload_success:
            ...
            log_stealth_interaction(
                "EXCEL_SAVE", 
                f"Mode:{'SINGLE' if single_file_mode else 'BATCH'}, Changes:{len(pending_changes)}"
            )
        ...
        log_stealth_interaction("EXCEL_SAVE_ERROR", str(e))
        return None
...
if st.session_state.drive_service is None:
    try:
        ...
        log_stealth_interaction("DRIVE_AUTH_SUCCESS", "Google Drive connected")
    except Exception as e:
        log_stealth_interaction("DRIVE_AUTH_ERROR", str(e))
...
if st.session_state.df is None and not st.session_state.excel_loaded:
    with st.spinner(f"📥 Loading Excel from Supabase ({BRAND_FOLDER})..."):
        ...
        if working_path:
            ...
            log_stealth_interaction("EXCEL_LOAD", "Loaded working file")
        else:
            original_path = get_original_excel_from_brand()
            if not original_path:
                log_stealth_interaction("EXCEL_LOAD_ERROR", "Original file not found")
                st.error(f"❌ Could not load Excel file from Supabase folder: {BRAND_FOLDER}")
                st.stop()
            
            df = pd.read_excel(original_path)
            st.sidebar.success("✅ Loaded original Excel")
            log_stealth_interaction("EXCEL_LOAD", "Loaded original file")
        ...
        if missing:
            log_stealth_interaction("EXCEL_VALIDATION_ERROR", f"Missing columns: {missing}")
        ...
        log_stealth_interaction("EXCEL_VALIDATION", f"Total:{len(df)}, Invalid:{len(invalid_rows)}")
...
if len(invalid_rows) == 0:
    log_stealth_interaction("COMPLETION", "All files processed")
...
with col_nav1:
    if st.button("⬅️ Previous") and st.session_state.index > 0:
        ...
        save_state_to_supabase()
        log_stealth_interaction("NAVIGATION", f"Previous to index {st.session_state.index}")
        st.rerun()
with col_nav3:
    if st.button("Next ➡️") and st.session_state.index < len(invalid_rows) - 1:
        st.session_state.index += 1
        save_state_to_supabase()
        log_stealth_interaction("NAVIGATION", f"Next to index {st.session_state.index}")
        st.rerun()
...
# Log current file being viewed
log_stealth_interaction("FILE_VIEW", f"Index:{st.session_state.index}, File:{row['Original Name']}")
...
with col_middle:
    ...
    with col_btn1:
        if st.button("💾 Save Change", use_container_width=True):
            ...
            save_state_to_supabase()
            log_stealth_interaction("FILE_EDIT", f"File:{row['Original Name']}, New:{new_proposed}")
    ...
    with col_btn2:
        if st.button("🔄 Reset", use_container_width=True):
            if row['Full Path'] in st.session_state.pending_changes:
                log_stealth_interaction("FILE_RESET", f"File:{row['Original Name']}")
    ...
    if len(st.session_state.pending_changes) >= 10:
        ...
        if st.button("💾 Save Batch Now", use_container_width=True, type="primary"):
            ...
            if out_fname:
                ...
                log_stealth_interaction("BATCH_SAVE", f"Saved {num_changes} changes")
...
with col_right:
    ...
    if file_meta and tmp_path:
        try:
            ...
            log_stealth_interaction("PREVIEW_ERROR", str(e))
...
if st.session_state.pending_changes:
    ...
    with col_action1:
        if st.button("💾 Save All & Upload to Supabase", use_container_width=True, type="primary"):
            ...
            if out_fname:
                num_changes = len(st.session_state.pending_changes)
                log_stealth_interaction("FINAL_BATCH_SAVE", f"Saved all {num_changes} changes")
                st.session_state.pending_changes.clear()
                save_state_to_supabase()
    ...
    with col_action2:
        if st.button("🗑️ Clear All Pending", use_container_width=True):
            log_stealth_interaction("CLEAR_ALL", f"Cleared {len(st.session_state.pending_changes)} changes")
            st.session_state.pending_changes.clear()
            save_state_to_supabase()
            st.rerun()
else:
